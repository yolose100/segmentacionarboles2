...
#dummy_input =Variable(torch.zeros(10, 3,128,128))
...
def train(epoch, phase='train'):
    ...
    for batch in pbar:
        ...
        probs_numpy = torch.squeeze(probs.cpu()).detach().numpy()
       # threshold  = threshold_otsu(probs_numpy)
        ...
        pbar.set_postfix({
            ...
            'metric': "%.09f" % (running_metric / total)
        })
