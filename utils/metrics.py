...
def calc_iou(actual, pred):
    ...
    print ("La rica Union"+str(union)+" La rica Intersection" + str( intersection) )
    iou_result = intersection / union if union != 0 else 0.
    print ("El rico resultado "+ str(iou_result))
    return iou_result
...
def dice(mask,preds):
    dice = ((preds [mask==1] ) ).astype(np.float).sum(axis=(1,2))*2.0 / ((mask).astype(np.float).sum(axis=(1,2))+(preds).astype(np.float).sum(axis=(1,2)))
    return dice
...
def pixel_accuracy(masks, preds):
    ...
    print (str(masks.shape))
    print (str((preds == masks).astype(np.float).sum(axis=(1,2))))  
    print (str(masks[0,:,:].size))
    return correct
